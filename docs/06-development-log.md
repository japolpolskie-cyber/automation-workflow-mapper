...
- Phase D Sprint 2 added pure deterministic Router and Binary Decision detectors. Router preserves three-or-more semantic outcomes and clarifies unclear or overlapping routing semantics; Binary Decision preserves explicit conditions, alternate actions, and semantic two-way labels. Stable source-offset IDs, non-overlap safeguards, false-positive coverage, and Workflow Brief metadata previews were verified without planner or runtime integration.
- The minimum usable Workflow Brief vertical slice added a pure draft assembler, isolated service, and experimental internal endpoint. It converts only Router and Binary Decision results into schema-validated, unlocked draft JSON, using clearly identified trigger/action scaffolding without invoking providers, persistence, graph compilation, platform translation, or the production Mapper pipeline.
- The internal Workflow Brief preview added an exact-path development page, guarded API response parsing, focused read-only result sections, loading/error/empty states, and direct-route isolation. It has no production navigation item and does not alter the existing Mapper application flow.
- Phase D Sprint 3A added deterministic Wait and active human Approval detection. Schedule, polling, reminder, retry, descriptive-status, automatic-validation, and notification false positives are excluded; same-scope approval waits are deduplicated. The internal assembler and preview now expose complete boundaries while unresolved details remain unlocked suggestions with clarification.
- The Router semantic correction added a strict internal routing fact with bounded clauses, exact offsets, three-or-more semantic outcomes, exclusivity confidence, and optional adjacent handling evidence. Router consumes these facts before its literal fallback; fan-out, sequential, collection, descriptive-list, destination-only, and binary wording remain excluded. Binary Decision, Wait, and Approval were not migrated, and no AI/provider behavior was added.
- The Wait semantic correction added strict bounded facts for natural duration, date, event, response, and approval boundaries, including explicit timeout outcomes and exact offsets. Wait consumes these facts before its explicit fallback and rejects schedule, polling, follow-up, retry, historical, and vague timing. Binary Decision and Approval remain unchanged; no AI/provider or production behavior was added.
