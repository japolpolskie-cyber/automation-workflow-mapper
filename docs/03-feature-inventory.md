...
| Capability-detection contract | Partially implemented | Phase D Sprints 1–2 provide the strict detection contract plus deterministic Router and Binary Decision detectors with exact evidence, semantic route labels, ambiguity handling, and temporary entity hints. An isolated internal vertical slice can assemble these results into validated draft Workflow Brief JSON. Other catalog functions remain undetected. |
| Internal draft Workflow Brief endpoint | Implemented | `POST /api/internal/workflow-brief/draft` runs only Router and Binary Decision detection, assembles schema-valid reviewable JSON with explicit draft scaffolding, and remains separate from production Mapper generation. |
