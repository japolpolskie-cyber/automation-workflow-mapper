...
export type ProjectCreationMode = 'requirements' | 'blank';

export function NewProjectDialog({ open, busy, template, onClose, onCreate }: { open: boolean; busy: boolean; template?: WorkflowTemplate | null; onClose: () => void; onCreate: (input: CreateProjectInput, mode: ProjectCreationMode) => Promise<void> }) {
  const [name, setName] = useState('');
  const [clientName, setClientName] = useState('');
  const [description, setDescription] = useState('');
  const [platform, setPlatform] = useState<Platform>('n8n');
  const [mode, setMode] = useState<ProjectCreationMode>('requirements');
  useEffect(() => {
    if (!open) return;
  ...
  const submit = async (event: FormEvent) => {
    ...
    await onCreate({ name: name.trim(), clientName: clientName.trim(), description: description.trim(), platform }, mode);
  };
  return <div className="dialog-backdrop" role="presentation" onMouseDown={(event) => event.target === event.currentTarget && onClose()}>
    <section className="dialog" role="dialog" aria-modal="true" aria-labelledby="dialog-title">
      ...
      <form onSubmit={(event) => void submit(event)}>
        {!template && <fieldset className="creation-mode-fieldset"><legend>How would you like to start?</legend><div className="creation-mode-row">
          <button type="button" className={mode === 'requirements' ? 'creation-mode selected' : 'creation-mode'} aria-pressed={mode === 'requirements'} onClick={() => setMode('requirements')}><strong>Analyze requirements</strong><span>Describe the process and let the analyzer draft it.</span></button>
          <button type="button" className={mode === 'blank' ? 'creation-mode selected' : 'creation-mode'} aria-pressed={mode === 'blank'} onClick={() => setMode('blank')}><strong>Create blank workflow</strong><span>Add, configure, and connect every step yourself.</span></button>
        </div></fieldset>}
        ...
        <div className="dialog-actions"><button type="button" className="button secondary" onClick={onClose}>Cancel</button><button type="submit" className="button primary" disabled={busy || !name.trim()}>{busy ? 'Creating…' : mode === 'blank' ? 'Open blank builder' : 'Create project'}</button></div>
      </form>
    </section>
  </div>;
}
