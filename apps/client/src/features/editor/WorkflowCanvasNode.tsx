...
export function WorkflowCanvasNode({ data, selected }: NodeProps<EditorNode>) {
  ...
  const routes = ['router', 'split'].includes(node.category)
    ? (data.routeHandles?.length ? data.routeHandles : ['route-1']).map((id, index) => ({
        id,
        label: data.platform === 'zapier' ? `Path ${index + 1}` : data.platform === 'make' ? `Route ${index + 1}` : `Output ${index + 1}`,
        top: `${((index + 1) / ((data.routeHandles?.length ?? 1) + 1)) * 100}%`,
      }))
    : [];
  const outcomes = node.category === 'human_approval'
  ...
  return <div className={`workflow-node category-${custom ? 'custom' : node.category} ${selected ? 'selected' : ''}`}>
    ...
    {node.category === 'end' ? null : decision ? <><Handle id="positive" type="source" position={Position.Right} style={{ top: '36%' }} className="flow-handle positive" title="TRUE" /><Handle id="negative" type="source" position={Position.Right} style={{ top: '72%' }} className="flow-handle negative" title="FALSE" /></> : routes.length ? routes.map((route) => <Handle key={route.id} id={route.id} type="source" position={Position.Right} style={{ top: route.top }} className="flow-handle route" title={route.label} />) : outcomes.length ? outcomes.map((outcome) => <Handle key={outcome.id} id={outcome.id} type="source" position={Position.Right} style={{ top: outcome.top }} className={`flow-handle ${outcome.className}`} title={outcome.label} />) : <Handle id="default" type="source" position={Position.Right} className="flow-handle" title="Continue" />}
  </div>;
}
