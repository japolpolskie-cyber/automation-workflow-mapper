import { comparePlatforms, generateImplementationChecklist, generateImplementationMarkdown } from '@awm/platforms';
...
function saveData(data: string, filename: string, mimeType: string) {
  ...
  link.href = data.startsWith('data:') ? data : URL.createObjectURL(new Blob([data], { type: mimeType }));
...
export function ComparisonExportPanel({ workflow, workflowSet, selectedWorkflowId, platform, canvasId, nodes, onClose }: ComparisonExportPanelProps) {
  ...
  const exportSurface = () => {
    const element = document.getElementById(canvasId)?.querySelector('.react-flow__viewport') as HTMLElement | null;
  ...
  const exportVisual = async (format: 'png' | 'svg' | 'pdf') => {
    ...
    try {
      ...
      if (format === 'svg') {
        saveData(await toSvg(surface.element, options), `${name}-workflow.svg`, 'image/svg+xml');
      } else {
        const png = await toPng(surface.element, { ...options, pixelRatio: 2 });
        if (format === 'png') saveData(png, `${name}-workflow.png`, 'image/png');
        else {
          const pdf = new jsPDF({ orientation: 'landscape', unit: 'pt', format: 'a4' });
          const width = pdf.internal.pageSize.getWidth(); const height = pdf.internal.pageSize.getHeight();
          pdf.setFontSize(9); pdf.text('Draft export — review required before platform implementation.', 24, 20);
          pdf.addImage(png, 'PNG', 24, 30, width - 48, height - 54, undefined, 'FAST');
  ...
  return <div className="assistant-backdrop"><section className="compare-export-panel">
    <header><div><span><Scale size={17} /></span><div><strong>Compare and export</strong><small>Platform fit and implementation handoff</small></div></div><button className="icon-button" aria-label="Close comparison" onClick={onClose}><X size={18} /></button></header>
    <nav><button className={tab === 'compare' ? 'active' : ''} onClick={() => setTab('compare')}>Platform comparison</button><button className={tab === 'export' ? 'active' : ''} onClick={() => setTab('export')}>Exports</button></nav>
    <div className="compare-export-content">{tab === 'compare' ? <>
      <div className="recommended-platform"><CheckCircle2 size={18} /><div><span>Recommended platform</span><strong>{comparison.entries.find((entry) => entry.platform === comparison.recommendedPlatform)?.platformName}</strong><p>{comparison.recommendationReasons[0]}</p></div></div>
      <div className="comparison-grid">{comparison.entries.map((entry) => <article className={entry.platform === comparison.recommendedPlatform ? 'recommended' : ''} key={entry.platform}>
        <header><strong>{entry.platformName}</strong><span>{entry.score}/100</span></header>
        <dl><div><dt>Steps</dt><dd>{entry.steps}</dd></div><div><dt>Branches</dt><dd>{entry.branches}</dd></div><div><dt>Complexity</dt><dd>{entry.complexity}</dd></div><div><dt>Usage</dt><dd>{entry.usage}</dd></div><div><dt>Maintainability</dt><dd>{entry.maintainability}</dd></div><div><dt>Flexibility</dt><dd>{entry.flexibility}</dd></div><div><dt>Skill required</dt><dd>{entry.skillRequired}</dd></div><div><dt>Validation</dt><dd>{entry.validationErrors} errors · {entry.validationWarnings} warnings</dd></div></dl>
      ...
      <div className="export-grid">
        ...
        <button disabled={Boolean(working)} onClick={() => void exportVisual('svg')}><Image /><strong>Canvas SVG</strong><span>Scalable workflow illustration</span></button>
        <button disabled={Boolean(working)} onClick={() => void exportVisual('pdf')}><Download /><strong>Canvas PDF</strong><span>Landscape visual handoff</span></button>
      </div>
      {working && <p className="export-working"><LoaderCircle className="spin" size={14} />Preparing {working.toUpperCase()} export…</p>}{error && <p className="assistant-error">{error}</p>}
    </>}</div>
  </section></div>;
}
