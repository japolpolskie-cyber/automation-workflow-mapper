...
export const useEditorStore = create<EditorState>((set, get) => ({
  ...
  autoLayout(direction = 'LR') { set((state) => { const graph = new dagre.graphlib.Graph().setDefaultEdgeLabel(() => ({})); graph.setGraph({ rankdir: direction, ranksep: 110, nodesep: 80, edgesep: 35 }); state.nodes.forEach((node) => graph.setNode(node.id, { width: 290, height: 184 })); state.edges.forEach((edge) => graph.setEdge(edge.source, edge.target)); dagre.layout(graph); return { ...record(state), nodes: state.nodes.map((node) => { const point = graph.node(node.id) as { x: number; y: number }; return { ...node, position: { x: point.x - 145, y: point.y - 92 } }; }) }; }); },
