...
const connectionSemantics = (sourceHandle: string | null | undefined): Pick<WorkflowConnection, 'sourcePort' | 'label' | 'branchLabel' | 'style' | 'condition' | 'routeType'> => {
...
export const useEditorStore = create<EditorState>((set, get) => ({
  ...
  connect(connection) {
    ...
    set((state) => {
      ...
      const id = crypto.randomUUID(); const domainConnection: WorkflowConnection = { id, sourceNodeId: source, targetNodeId: target, ...connectionSemantics(connection.sourceHandle), targetPort: connection.targetHandle ?? 'input', mappings: [] };
  ...
  reconnect(edge, connection) {
    ...
    set((state) => {
      ...
      const updated: WorkflowConnection = { ...current, sourceNodeId: source, targetNodeId: target, ...connectionSemantics(connection.sourceHandle), targetPort: connection.targetHandle ?? 'input' };
