...
export class UnifiedPlannerRuntime implements PlannerRuntime {
  ...
  public async execute(provider: AnalysisProvider, objective: string, platform: Platform, analysis: DetectedProcessSummary, productionWorkflow: CanonicalWorkflow, signal?: AbortSignal): Promise<PlannerRuntimeResult> {
    ...
    if (this.mode === 'mock') return { plannerShadow: emptyComparison('completed', null, 0, null) };
    if (this.mode === 'shadow') {
      if (!this.shadowRuntime) return {};
      return { plannerShadow: await this.shadowRuntime.compare(provider, objective, platform, analysis, productionWorkflow, signal) };
    }
    if (this.deterministicRuntime) {
      ...
      return { plannerShadow: plannerShadowComparisonSchema.parse(comparison) };
    }
    if (!this.distributedRuntime) return { plannerShadow: emptyComparison('failed', 'Distributed planner runtime is not configured.', 0, 'unknown_provider_error') };
    const started = performance.now();
