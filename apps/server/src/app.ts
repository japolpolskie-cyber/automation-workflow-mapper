...
import { selectPlannerRuntimeMode, UnifiedPlannerRuntime } from './planner/planner-runtime-service.js';
...
export async function buildApp(environment: Environment) {
  const app = Fastify({ logger: environment.NODE_ENV === 'test' ? false : { level: environment.LOG_LEVEL }, genReqId: () => crypto.randomUUID(), bodyLimit: 1_048_576 });
  await app.register(helmet);
  await app.register(cors, { origin: environment.CLIENT_ORIGIN, methods: ['GET', 'POST', 'PATCH', 'DELETE'] });
  ...
  const provider = environment.AI_PROVIDER === 'openai'
    ? new OpenAIAnalysisProvider({ apiKey: environment.OPENAI_API_KEY ?? '', baseUrl: environment.OPENAI_BASE_URL, model: environment.OPENAI_MODEL })
    : environment.AI_PROVIDER === 'ollama'
      ? new OllamaAnalysisProvider({ baseUrl: environment.OLLAMA_BASE_URL, models: environment.OLLAMA_MODELS.split(',').map((model) => model.trim()).filter(Boolean) })
      : new LocalAnalysisProvider();
  await app.register(healthRoutes, { prefix: '/api' });
  await app.register(documentRoutes(new DocumentService()), { prefix: '/api' });
  const scopeIntelligence = environment.K3_SCOPE_INTELLIGENCE ? new ScopeIntelligenceService(environment.K3_KNOWLEDGE_BUDGET) : null;
  ...
  const distributedRuntime = new P3ShadowExperimentService({
    ...
    cacheEnabled: environment.P3_CACHE_ENABLED && environment.P2_DISTRIBUTED_PLANNER_CACHE,
  }, new ProviderPlannerStageRunnerFactory(provider));
  const stageCGrounding = environment.STAGE_C_NODE_GROUNDING ? new StageCGroundingService({
    enabled: true,
  ...
  const stageDGrounding = environment.STAGE_D_EDGE_GROUNDING ? new StageDGroundingService({
    enabled: true,
  ...
  const deterministicRuntime = new P4DeterministicPlannerService(new ProviderPlannerStageRunnerFactory(provider), undefined, undefined, stageCGrounding, stageDGrounding);
  await app.register(analysisRoutes(new AnalysisService(repository, provider, undefined, scopeIntelligence, new UnifiedPlannerRuntime(runtimeMode, shadowRuntime, distributedRuntime, deterministicRuntime))), { prefix: '/api' });
  await app.register(platformRoutes(new PlatformService(repository)), { prefix: '/api' });
  await app.register(assistantRoutes(new AssistantService(repository)), { prefix: '/api' });
  await app.register(projectRoutes(service), { prefix: '/api' });
  app.setNotFoundHandler((request, reply) => reply.code(404).send({ success: false, data: null, error: { code: 'NOT_FOUND', message: 'Route not found.' }, meta: { requestId: request.id } }));
  app.setErrorHandler((error, request, reply) => {
    ...
    const multipartError = typeof error === 'object' && error !== null && 'code' in error && error.code === 'FST_REQ_FILE_TOO_LARGE';
    const statusCode = analysisError ? error.statusCode : documentError ? error.statusCode : validation || multipartError ? 400 : 500;
    const code = analysisError ? error.code : documentError ? error.code : multipartError ? 'FILE_TOO_LARGE' : validation ? 'VALIDATION_ERROR' : 'INTERNAL_ERROR';
    const message = analysisError ? error.message : documentError ? error.message : multipartError ? 'The file exceeds the 10 MB upload limit.' : validation ? 'The request was invalid.' : 'An unexpected error occurred.';
    return reply.code(statusCode).send({ success: false, data: null, error: { code, message }, meta: { requestId: request.id } });
  });
  return app;
}
