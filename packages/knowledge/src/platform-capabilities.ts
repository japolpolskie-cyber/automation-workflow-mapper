...
const seeds: Record<CanonicalFunctionId, PlatformSeeds> = {
  ...
  iterator: {
    n8n: { implementation: 'Split Out for array-to-items; Loop Over Items only for batching or explicit loop control' },
    make: { implementation: 'Iterator module', limitation: 'A Router creates conditional branches and must not be used as a collection iterator.' },
    zapier: { implementation: 'Looping by Zapier — Create Loop From Line Items', limitation: 'Every loop iteration consumes downstream tasks. Formatter may prepare line items, but it is not the iterator.' },
  },
