...
import { isAiAttachmentConnection, isAiAttachmentNode } from './ai-agent-attachments.js';
...
export function validateWorkflow(workflow: CanonicalWorkflow, platform: Platform, platformIssues: PlatformValidationIssue[] = []): WorkflowValidationResult {
  ...
  for (const edge of workflow.connections.filter((item) => !isAiAttachmentConnection(item))) { outgoing.set(edge.sourceNodeId, (outgoing.get(edge.sourceNodeId) ?? 0) + 1); incoming.set(edge.targetNodeId, (incoming.get(edge.targetNodeId) ?? 0) + 1); }
  const signatures = new Map<string, string>();
  for (const node of workflow.nodes) {
    if (isAiAttachmentNode(node)) continue;
  ...
  const applications = new Set(executionNodes.map((node) => node.service).filter(Boolean)).size;
  const apiCalls = executionNodes.filter((node) => node.category === 'api_request').length;
  const humanSteps = executionNodes.filter((node) => node.category === 'human_approval').length;
  const configuredPercent = executionNodes.length ? Math.round(executionNodes.reduce((sum, node) => sum + node.configurationCompleteness, 0) / executionNodes.length) : 0;
  const complexityScore = Math.min(100, executionNodes.length * 2 + workflow.branches.length * 6 + apiCalls * 5 + applications * 2 + humanSteps * 3 + workflow.risks.filter((risk) => ['high', 'critical'].includes(risk.severity)).length * 5);
  ...
  return { valid: !issues.some((issue) => issue.severity === 'error'), issues, complexityScore, complexity, usageEstimate, statistics: { nodes: executionNodes.length, connections: workflow.connections.filter((edge) => !isAiAttachmentConnection(edge)).length, applications, branches: workflow.branches.length, apiCalls, humanSteps, configuredPercent } };
}
